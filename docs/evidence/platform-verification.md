...
| 11 | Marketplace paid settlement tx | Real Base USDC payment + execution | Wallet funded at 0.50 Base USDC. Two 0.01 x402 transfers settled successfully: `0xabbe77bc…3563` and `0x87f5c75f…e71f`. `wallet-snapshot-base` failed after charging; alternate `defi-onchain-intelligence-base` completed. Wallet ended at 0.48 USDC. | **PASS** (platform settlement; one listing bug) |
...
| 13 | Agentic wallet install | `skill install` + `wallet add` | Skill installed; MCP `info`/`balance` auto-provision successfully. Balance after two paid calls is 0.48 Base USDC. | **PASS** |
...
2. **Settlement is proven** by two successful Base USDC receipts in `phase11-paid-settlement-2026-07-22.json`; the first listing failed after charging and the alternate listing completed.  
3. Keep W3 fee state machine with `X402` / `MPP` / `ESCROW_FALLBACK`. Do not list W3 until its own paid execution semantics are proven; generic x402 settlement does not certify W3 behavior. Escrow fallback remains the rehearsal path.  
