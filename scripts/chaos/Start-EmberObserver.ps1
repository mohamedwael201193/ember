# Start the isolated Primary Observer process (no Docker).
param(
...
$env:PRIMARY_OBSERVER_PORT = "8788"

$workDir = Join-Path $RepoRoot "services\primary-observer"
$proc = Start-Process -FilePath "node" -ArgumentList "dist/main.js" -WorkingDirectory $workDir -PassThru -WindowStyle Hidden
Start-Sleep -Seconds 2
$healthy = $false
try {
  $r = Invoke-WebRequest -Uri "http://127.0.0.1:8788/healthz" -UseBasicParsing -TimeoutSec 5
